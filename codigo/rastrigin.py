...
def decodificar(individuo):
    ...
    for i in range(N_DIM):
        ...
        valor_inteiro = int("".join(str(b) for b in bloco_binario), 2)
...
def selecao_torneio(populacao, aptidoes, k=3):
    ...
    selecionados.sort(key=lambda item: item[1], reverse=True) # Maior aptidão é melhor
...
def selecao_roleta(populacao, aptidoes):
    ...
    pick = random.uniform(0, soma_aptidoes)
    atual = 0
...
def executar_ag_rastrigin(pc, pm, silencioso=True):
    ...
    while avaliacoes < MAX_EVALS:
        ...
        # Inverte aptidão, mas evita divisão por zero caso encontre o ótimo perfeito
        aptidoes = [1.0 / (1.0 + v) if v >= 0 else 1.0 for v in valores_objetivo]
        avaliacoes += POP_SIZE
        ...
        melhor_idx = valores_objetivo.index(min(valores_objetivo))
        nova_populacao.append(list(populacao[melhor_idx]))
